...
import unittest
import numpy as np
from moule_svg_cadquery import normalize_svg_fill
...
class TestSVGFlattenSimilarity(unittest.TestCase):
    ...
    def test_flattened_similarity(self):
        ...
        print_svg_attrs(normalized_svg, "SVG normalisé")

        src_png_path = os.path.join(self.debug_dir, "raster_source.png")
        normalized_png_path = os.path.join(self.debug_dir, "raster_normalized.png")
        src_arr = svg_to_array_and_save(self.svg_file_path, src_png_path, size=256)
        normalized_arr = svg_to_array_and_save(normalized_svg, normalized_png_path, size=256)

        # Calcule la similarité (IoU et différence de pixels)
        intersection = np.logical_and(src_arr, normalized_arr).sum()
        union = np.logical_or(src_arr, normalized_arr).sum()
        iou = intersection / union if union > 0 else 0
        diff = np.abs(src_arr - normalized_arr).sum()
